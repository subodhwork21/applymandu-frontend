...
export class FetchErrorAdmin extends Error {
    status: number;
...
export async function fetchApiAdmin(url: string, options: RequestInit = {}) {
    const response = await fetch(url, {
      headers: {
        'Content-Type': 'application/json',
  ...
  // Handle server error (500) - throw an error
  if (response.status === 500) {
    throw new FetchErrorAdmin(500, "Something went wrong");
  }
...
//base fetcher and admin fetcher for swr


export async function baseFetcherAdmin(
  url: string,
  options: RequestInit = {}
) {
  try{
    ...
    const fullUrl = `${baseUrl}${url}`;
    const fetch = await fetchApiAdmin(fullUrl, options);
...
const defaultFetcherAdmin = async <T>(url: string): Promise<T> => {
  const baseUrl = process.env.NEXT_PUBLIC_API_URL;
  const response = await fetch(baseUrl + url, {
    headers: {
      "Content-Type": "application/json",
